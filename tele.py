...
def handle_gender(call):
    ...
    # Ask for the phone number after gender selection
    msg = bot.send_message(chat_id, "Будь ласка, введіть ваш номер телефону:")
...
def save_user_data(chat_id):
    ...
    cursor.execute('''
        INSERT OR IGNORE INTO User (
            chat_id, first_name, last_name, birthdate, gender, phone
        ) VALUES (?, ?, ?, ?, ?, ?)
    ''', (
        chat_id,
        data.get('first_name', ''),
